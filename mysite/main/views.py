...
def access_top_shounen(request):
    try:
        with connection.cursor() as cursor:
            cursor.execute('''SELECT manga_id, title, type, chapters, status, score, main_picture FROM manga WHERE sfw = 'True' AND demographics LIKE '%Shounen%'AND main_picture IS NOT NULL AND score IS NOT NULL ORDER BY score DESC LIMIT 1000''')
            rows = cursor.fetchall()
            if rows:
                mangas = []
                for row in rows:
